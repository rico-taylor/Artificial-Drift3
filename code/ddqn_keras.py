...
class DDQNAgent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size,
                 input_dims=16, epsilon_dec=0.999995, epsilon_end=0.10,
    ...
    def choose_action(self, state):
        ...
        if rand < self.epsilon:
            action = (np.random.rand(self.n_actions) > 0.5).astype(int).tolist()
        else:
            actions = self.brain_eval.predict(state)
            action = (actions[0] > 0.5).astype(int).tolist()
    ...
    def learn(self):
        if self.memory.mem_cntr > self.batch_size:
            ...
            q_eval = self.brain_eval.predict(state)
            q_pred = self.brain_eval.predict(state)

            batch_index = np.arange(self.batch_size, dtype=np.int32)
            max_actions = np.argmax(q_eval, axis=1)

            # Convert boolean action list to indices
            action_indices = np.argmax(action, axis=1)

            q_target = q_pred.copy()
            q_target[batch_index, action_indices] = reward + self.gamma * q_next[batch_index, max_actions] * (1 - done)
...
class Brain:
    ...
    def createModel(self):
        ...
        model.add(tf.keras.layers.Dense(self.NbrActions, activation='sigmoid'))
