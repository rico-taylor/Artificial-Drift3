...
class car:
  def __init__(self, img, starting_pt, ai):
    # image - image file for the car
    # starting_pt - tuple, coordinates for the starting position of the car
    # ai - boolean, whether or not the car is an AI racer or a user racer
    
    #Car image and position on the screen
    ...
    self.car_start_x =  starting_pt[0]/1920 *window.width
    self.car_start_y =  starting_pt[1]/1080 *window.height
    self.car = sprite.Sprite(self.car_image, self.car_start_x, self.car_start_y)
    self.car.scale = 0.1*scale_factor
    self.car.rotation = 260
    ...
    #NOTE: AI SPECIFIC VARIABLES FOR AI section
    ...
    def stopwatch(self):
      ...
      if self.lapCompleted == True:
        self.lap_list.append(float("{:#.2f}".format(elapsed)))
        self.lapCompleted = False
      ...
      elapsed = self.current - self.start
      self.swap = False
    ...
    def lineChecks(self, input_line, car_hitbox): #STILL NEEDS PLAYER THINGS
      if car_hitbox == self.sprite_hitbox:
        noStart = False
        if input_line == timer1:
          ...
          if self.checkerList[timerLineList.index(input_line)-1] == True or self.checkerList[timerLineList.index(input_line)-2] == True or self.checkerList[timerLineList.index(input_line)-3] == True or self.checkerList[timerLineList.index(input_line)-4] == True or self.checkerList[timerLineList.index(input_line)-5] == True or self.checkerList[timerLineList.index(input_line)-6] == True:
            self.swap = True
            started = False
          ...
          if self.checkerList[timerLineList.index(input_line)-1] == True or self.checkerList[timerLineList.index(input_line)-2] == True or self.checkerList[timerLineList.index(input_line)-3] == True or self.checkerList[timerLineList.index(input_line)-4] == True or self.checkerList[timerLineList.index(input_line)-5] == True or self.checkerList[timerLineList.index(input_line)-6] == True:
            ...
            self.checkerList[timerLineList.index(input_line)] = True
...
#player car image = "images/car.png"
#starting_pt = (1020,185)
...
def find_intersection(line1, line2, x,y): #x,y is self.car.x, self.car.y
    ...
    if determinant == 0:
        ...
        if (min(x1, x2) <= xi <= max(x1, x2) and min(y1, y2) <= yi <= max(y1, y2) and
            ...
            dist = distance_points((x, y), (xi,yi))
            return (xi, yi, dist)
        else:
            return None
