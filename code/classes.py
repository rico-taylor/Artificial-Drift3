...
class entryScreen:
  ...
  #window
    self.entryWindow = pyglet.window.Window(caption="Entry")
    self.entryWindow.set_fullscreen(True)

    self.entryDisplay = pyglet.graphics.Batch()

    #entry images
    logo_img = image.load("images/logo_finished.png")
    logo_img.anchor_x = logo_img.width//2
    logo_img.anchor_y = logo_img.height//2
    logo = sprite.Sprite(logo_img, x=self.entryWindow.width//2, y=self.entryWindow.height//2 +100, batch=self.entryDisplay)

    play_img = image.load("images/text_play.png")
    play_img.anchor_x = play_img.width//2
    play_img.anchor_y = play_img.height//2
    playButton = sprite.Sprite(play_img, x=self.entryWindow.width//2, y=self.entryWindow.height//2 -200, batch=self.entryDisplay)
    ...
    triangle1 = sprite.Sprite(triangle_img, x=100, y=100, batch=self.entryDisplay)
    triangle1.rotation = 30
    triangle1.scale = 0.3

    def on_draw():
      self.entryWindow.clear()
      self.entryDisplay.draw()
